...
class Ellipse:
    ...
    def get_quarter(self):
        ...
        middle_point_ellipse = [p.Point(0 + self.offset_x, self.radius_y + self.offset_y, str(k))]
        k = 1
        ...
        while 2*(self.radius_y**2)*x_k <= 2*(self.radius_x**2)*y_k:
            ...
            y_k = new_values[1]
            k += 1
            middle_point_ellipse.append(p.Point(new_values[0]+self.offset_x, new_values[1]+self.offset_y, str(k)))
        ...
        middle_point_ellipse.append(p.Point(x_k + self.offset_x, y_k + self.offset_y, str(k)))
    ...
    def get_dimensions(self):
        if self.radius_y > self.radius_x:
